...
const ShadowQuiz = () => {
  ...
  const handleAnswer = useCallback(async (questionId: number, optionId: string, shadow: Record<string, number>) => {
    ...
    if (currentQuestion < currentQuestions.length - 1) {
      ...
      // Show loading state while calculating results
...
*If this problem persists, there may be an issue with the Claude API configuration.*`;
  ...
  if (currentScreen === 'quiz') {
    // Show loading skeleton if calculating results
    if (isLoadingResults) {
  ...
  if (currentScreen === 'results') {
    return (
      <ResultsScreen
        ...
        onSetUserPrefs={setUserPrefs}
        onSetCurrentScreen={(screen: string) => setCurrentScreen(screen as any)}
        onOpenDeepAnalysis={openDeepAnalysis}
        onOpenChat={openChat}
        onOpenJournal={openJournal}
        onOpenExercises={openExercises}
        onOpenGuide={openGuide}
        onOpenReAnalysis={openReAnalysis}
        onRestart={restart}
        onSetShowFeedback={setShowFeedback}
        onSetUserQuestion={setUserQuestion}
        onHandleAskClaude={handleAskClaude}
        onSetShowConversationHistory={setShowConversationHistory}
        onCreateJournalFromConversation={createJournalFromConversation}
        onCreateExerciseFromConversation={createExerciseFromConversation}
      />
    );
  }
  ...
  if (currentScreen === 'exercises') {
    ...
    return (
      <IntegrationExercises 
        ...
        onNavigateToDeepAnalysis={() => setCurrentScreen('deepanalysis')}
      />
    );
  }
